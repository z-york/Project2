...
{
	if (arcLength->value()) {
		calcArcLength((float)speed->value());
	} else {
...
float calcDistance(Pnt3f a, Pnt3f b) {
	float x = powf((b.x - a.x), 2);
	float y = powf((b.y - a.y), 2);
	float z = powf((b.z - a.z), 2);
	return sqrtf(x + y + z);
}

void TrainWindow::calcArcLength(float speed) {
	float step = .01;
	float distance = 0;
	float currU = world.trainU;
	Pnt3f next;
	Pnt3f curr = trainView->getPos(currU);

	//Move ~speed distance in a step
	while (distance < (speed)) {
		currU += step;
		next = trainView->getPos(currU);
		distance = calcDistance(curr, next);
	}
